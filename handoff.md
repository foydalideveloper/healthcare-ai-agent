...
2. **Deferred (from `handoff_2026-06-02.md` §5 — not active):**
   ...
   - **Visual recovery for KRX individual stocks + Hana Bank multi-currency board** — requires `ocr_preprocessor.py` changes (OFF-LIMITS today). On the KBS clip the OCR simply doesn't capture 현대차 / 삼성전기 / EUR-USD / CNH-KRW / CAD-KRW / S&P 500 / SK하이닉스-2,328,000 — the dense small-text financial boards come back as mojibake. Aggregator-side recovery is **proven infeasible** (Option B abandoned 2026-06-04 — see ❌ entry above; do NOT retry a post-processing scan). The real fix, when a session is explicitly authorized to touch the OCR pipeline: (a) higher-DPI capture / 3-4× upscale for dense small-text panels, (b) DBSCAN-eps tuning to separate dense ticker rows, (c) possibly PaddleStructure / table-specific OCR for the KRX board. Est. 4-6 h. (added 2026-06-04.)
