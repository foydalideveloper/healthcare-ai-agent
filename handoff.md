...
**✅ Completed (2026-06-05, `3edf2b6`): Self-Improvement Job 2 — Auto-Labeled Dataset Builder (shipped, read-only).**
Turns Job 1's cross-arm consensus into a versioned, training-ready JSONL dataset — the foundation for Job 3 (LoRA fine-tune of local Gemma 4). Read-only over DB + video files; NO production code changed; no new packages (PyAV + PIL already in venv).
- **Isolated under NEW `backend/training/`** (6 modules + 4 test files): `event_query.py` (flatten + census + per-chunk consensus), `frame_extractor.py` (PyAV even-split sampling), `label_filter.py` (high/medium tiers), `dataset_writer.py` (JSONL + SHA256 split + manifest), `dataset_builder.py` + `cli.py`. Output → `backend/training_data/<version>/` (**gitignored**, regenerable).
- **Per-CHUNK consensus (not per-video):** Job 1 computes consensus over the whole video; Job 2 needs per-`chunk_idx` granularity to pair (inputs↔labels). Same algorithm, same determinism property, called with `min_agreement_count=None` (adaptive 67%).
- **⚠️ INVOCATION (non-obvious — document loudly):** run **from `backend/`** as `..\.venv\Scripts\python.exe -m training.cli --version v1.0.0`. Running `-m backend.training.cli` from repo root FAILS — `from app.database` won't resolve. If you see `ImportError: app.database`, you're in the wrong cwd.
- **Phase-0 reality corrections (verified against live DB):** `observed_facts`/`video_extraction`/`audio_extraction` are NOT columns — they live in `raw_event` JSONB and need the endpoint's flatten-first-wins (selecting them as a column 400s); group by `chunk_idx` (start_sec/end_sec are NULL); even-split frame windows `start=(idx/n)*duration`; `get_db_admin` from `app.database`; `async def` throughout.
- **Determinism (CRITICAL for Job 3 reproducibility):** arms sorted before clustering; OCR/transcript aggregation `set → sorted list` (a raw set→list ordering was a latent non-determinism bug — 2nd one caught this flywheel, after Job 1's clustering); train/val by `SHA256(seed:example_id)`. **Verified byte-identical (SHA256 match) across a v1.0.0→v1.0.1 rebuild.**
- **v1.0.0 stats (thin start, auto-grows):** the 109 raw "multi-arm" clips are mostly old food/activity events with no `observed_facts`; filtering to ≥2 arms WITH facts → 6 VLM clips, only the test clip has a present video → **2 examples** (its 2 chunks: chunk0 = 6 med, chunk1 = 2 high + 1 med), 1 train / 1 val, 32 frames, 46 MB. Manifest tagged `first_build` + `thin_start`. Skips tracked in `manifest.generation_metadata` (no_video 3 / no_consensus 1 / insufficient_labels 3).
- **Foundation for Job 3:** JSONL `(frame_paths + transcript + ocr_text) → (high_facts, medium_facts)`; 80/20 split; "overfit on 1 example" debugging possible now; rebuild reproduces the dataset byte-for-byte. 28 training + 46 services tests pass; all v3.5 gates intact (consensus 9, 131 facts, audio_only_terms 4, KOSPI multi-value, Option A backfill).

**✅ Completed (2026-06-05, `3e6b8a1`): Self-Improvement Job 1 — Cross-Arm Consensus Enrichment (shipped, 4 commits).**
The first of a 3-job self-improvement flywheel. Compares per-arm `observed_facts` for one source video and surfaces the facts ≥~67% of the arms independently agree on, each tagged with a cross-arm confidence score ("every fact comes with a cross-model agreement score").
- **Architecture — purely aggregator-level**, no extraction-pipeline / LLM-prompt / watcher changes; single-arm reports unchanged. NEW `backend/app/services/consensus_enricher.py` (stdlib only: `re`, `difflib`). `SequenceMatcher` fuzzy clustering at **0.75** similarity; **adaptive threshold** `min_agreement_count = max(2, round(arms × 0.67))` (4 arms→3/75%, 6 arms→4/67%). Schema **v3.4 → v3.5** (additive: `consensus_observed_facts` + 6 `metrics.consensus_*`).
- **Determinism property (the key catch):** the result is a function of the input **SET**, not input order — clustering canonicalizes by sorting arms by id and seeding clusters longest-fact-first. Verified stable (9/9/9) under shuffled input. This is the stability the Job 2 labeled dataset depends on.
- **Commit trail:** `1dffd4d` Part A (consensus_enricher.py + 16 unit tests) **+** Part B (aggregator integration + adaptive threshold + determinism, committed together) · `b2fe80a` Part C (Word doc section, adaptive badges) · `3e6b8a1` Part D (frontend modal + TS types).
- **Visual hierarchy (consistent Word + frontend):** High ≥83% = green `#1A7F37` / `bg-green-100 text-green-700`; Medium 67–83% = amber `#B8860B` / `bg-amber-100 text-amber-700`; below 67% not rendered (filtered at compute time).
- **Test-clip result (4-arm coverage: gemma4 + 3 Gemini; qwen+llama4 absent for this clip):** 9 consensus facts (2 high 4/4, 7 medium 3/4). `all_observed_facts` 131 **unchanged** (additive verified); all v3.4 preserve gates intact (KOSPI/KOSDAQ/USD-KRW multi-value, audio_only_terms=4, Option A backfill). 16/16 unit tests; 46/46 services suite; tsc 0 new errors; `/lifelog` compiles 200.
- **Scales to 6/8/N arms with no code change** — `arms_compared` is a runtime metric; when qwen+llama4 populate future clips, consensus uses them automatically. Built on the 4 present per the no-re-extract constraint (overrode Phase-0-step-2's "run missing arms" with the harder "operate on existing DB data only" DON'T).
- **Foundation for the flywheel:** Job 2 (auto-labeled dataset) uses `consensus_observed_facts` as ground-truth labels; Job 3 (LoRA fine-tune of local Gemma 4) trains on Job 2's data. See deferred items below.

**✅ Completed (2026-06-05): OCR Engine Evaluation (evaluation-only, NO production change, uncommitted under `backend/ocr_eval/`).**
- Settled the open "different tooling, not tuning" question at the engine level. Built a side-by-side harness (`backend/ocr_eval/`) and benchmarked 3 free engines on the two failure frames (f07 KRX Korean board, f08 Hana Latin currency board). **Result:** PaddleOCR baseline **16/22 (73%)** · PaddleStructure(en) 14/22 (64%) · **EasyOCR(ko+en) 11/22 (50%) — LOSES on both frames.**
- **EasyOCR rejected** (the `['ko','en']` single-model hope): 58% Korean / 40% Latin, slower, and recovered **0/4** of the flagged ceiling tokens (현대차/681,000/삼성전기/SK스퀘어) baseline missed. **Korean KRX ceiling = CASE δ: no free engine beats PaddleOCR** → 4th instance of the capture-layer-ceiling pattern (now: not free *tooling* either). **Latin currency board = CASE β:** PaddleStructure(en) wins f08 (90% vs 70%, faster) — an optional free, content-routed hybrid.
- **Recommendation (full matrix in `backend/ocr_eval/RECOMMENDATION.md`):** no swap; for Korean escalate to **Naver Clova OCR (paid)** eval [needs `CLOVA_OCR_API_KEY`, Phase D skipped] or **Mentra Live** hardware; optionally add a PaddleStructure(en) route for Latin currency boards (integration session decides). Resume doc: `handoff_ocr_eval_2026-06-05.md` (§15 = final results).
- **Env note:** installed `easyocr 1.7.2` **with `numpy==1.26.4` pinned** (a plain `pip install easyocr` would bump numpy→2.4.6 and break the paddle/torch/ctranslate2 DLL stack); torch stayed 2.11.0+cu128. Also added ninja, opencv-python-headless 4.11.0.86, python-bidi. Full DLL stack verified healthy before/after; production untouched.
...
- **`da20d0a` — Audio preprocessing flipped to DEFAULT-OFF (content preservation, 2026-06-04).** Option C (`85bf21f`) was found to **silently drop/substitute quieter legitimate speech**, not just denoise: on the test clip the analyst-quote sentences **`3년에서 5년`** (supply-contract extension) and **`사이클 장기화`** (prolonged cycle) — plus the whole `공독` supply-shortage sentence — vanished from the preprocessed transcript (443 chars, content gone) vs raw audio (492 chars, all present). **User priority: 100% audio-content preservation, accepting Whisper errors** (승리/심리, 크룸/흐름, 공독/공급, 붙습니다/컸습니다) as the trade. Change (ONLY the `whisper_transcribe` gate in `_lifelog_test.py`): preprocessing is now **DEFAULT-OFF**; enable via **`ENABLE_AUDIO_PREPROC=1`**; legacy **`SKIP_AUDIO_PREPROC=1`** still works and WINS over ENABLE. `audio_preprocessor.py` module + noisereduce/soundfile deps untouched (kept for future content-safe opt-in). Verified: default→raw 492-char transcript w/ both analyst sentences, audio_facts 10, all v3.4 gates intact.
  - **⚠️ Verification lesson (important for ALL future STT preprocessing work):** transcript **LENGTH is NOT a valid quality metric** — content loss here was disguised by near-identical length (443 vs 441/492). The original Option C report (`85bf21f`) under-weighted this: it verified "~90% of baseline length" without checking for **substituted/dropped speech segments**. The failure mode is specifically **dropping/substituting quiet speech**, not shortening. **Future preprocessing changes require content-presence gates (specific expected phrases), not length checks.**
...
2. **Deferred (from `handoff_2026-06-02.md` §5 — not active):**
   - **🔜 Self-Improvement Job 2: Auto-labeled dataset builder** — use `consensus_observed_facts` (≥ adaptive-threshold agreement, Job 1) as ground-truth labels for video chunks. Build a SQLite/Parquet dataset of (video frames + transcript) → consensus_facts pairs; tag with content type, language, scene complexity. Foundation for Job 3 LoRA fine-tuning. Depends on Job 1 (shipped). Est. 6–8 h. (added 2026-06-05.)
   - **🔜 Self-Improvement Job 3: LoRA fine-tuning of local Gemma 4 26B A4B** using Job 2's `backend/training_data/v1.0.0/` dataset — Korean financial + lifelog specialization. Goal: local Gemma matches Gemini 3.1 Pro quality on target content; production switch to the fine-tuned local primary arm cuts cloud cost ~80%. Est. 12–16 h. (added 2026-06-05, updated post-Job-2.)
     - **Prereqs:** PEFT (LoRA) lib — WILL need install (clear the DLL-safety bar like noisereduce did); GPU RTX (32GB) OK for LoRA on 26B (full FT would need more); dataset must grow first (see below).
     - **Format ready:** load `train.jsonl`/`val.jsonl`; each example = `(frame_paths + transcript + ocr_text) → (high_confidence_facts, medium_confidence_facts)`; frame_paths are relative to the version dir. Rebuild is byte-identical (reproducible training).
     - **Success criteria:** fine-tuned local Gemma emits consensus-quality `observed_facts` on HELD-OUT clips; val loss converges; on production-switch, ~80% Gemini cost cut.
     - **⛔ Gate:** do NOT start real training until v1.0.0 grows to **≥20 examples** — current 2 examples only support an "overfit on 1" loop sanity-check, not meaningful fine-tuning. The dataset auto-grows as more multi-arm v3.4+ clips WITH videos accumulate; just re-run `training.cli`.
   - **Job 1 browser interactive smoke test (pending user visual confirmation)** — on `localhost:3000/lifelog`. All automatable checks pass (tsc 0 new errors, `/lifelog` compiles 200, data contract matches); manual check confirms the green/amber confidence pills render, mobile wrap at 375px, and no F12 console errors. Any issue is a clean follow-up CSS commit, not a blocker. (added 2026-06-05.)
   ...
   - Audio quality — external mic / Mentra Live migration to fix Korean transcript errors. **(2026-06-04: noisereduce preprocessing was tried (`85bf21f`) then flipped DEFAULT-OFF (`da20d0a`) — it dropped legitimate quiet speech. Hardware mic upgrade remains the real fix for the phonetic-limit errors.)**
   - **Audio preprocessing content-safe tuning** — investigate why noisereduce removes/substitutes quiet speech alongside noise, so it can eventually be re-enabled by default. Options: (a) lower `prop_decrease` 0.85→~0.4; (b) `stationary=False` adaptive denoising; (c) WebRTC-VAD gate so denoise fires only during non-speech; (d) alternative libs (DeepFilterNet, rnnoise). **DUAL-CONSTRAINT success criteria — BOTH must hold on the test clip `20260528144922838.mp4` before any default-ON re-enable: (1) 100% speech preserved — the verified-vulnerable phrases `3년에서 5년` AND `사이클이 장기화` MUST be present in the transcript; (2) at least 1 Korean error corrected (e.g. 심리/흐름). Verify by CONTENT-PRESENCE, never by transcript length.** Until both hold simultaneously, keep `ENABLE_AUDIO_PREPROC` opt-in only. (added 2026-06-04.)
...
- **Default-OFF since 2026-06-04 (`da20d0a`):** audio preprocessing no longer runs by default (it dropped legitimate quiet speech). The deps are only needed for the **opt-in** path: set **`ENABLE_AUDIO_PREPROC=1`** to enable. `SKIP_AUDIO_PREPROC=1` is the legacy override and WINS over ENABLE. If the deps are missing, the opt-in path degrades gracefully to raw audio.
...
- FastAPI `--reload` + frontend hot-reload pick up aggregator/exporter/page edits — no restart needed. *(If you start uvicorn WITHOUT `--reload` to run a test, restart it after editing `full_report_aggregator.py` / `word_exporter.py` or it serves stale code — bit us briefly in the 2026-06-05 consensus session.)*
- **`consensus_enricher.py` (added 2026-06-05, Job 1):** aggregator-level PURE module (stdlib only). Future Job 2/3 sessions MAY modify it, but the **determinism property must be preserved** — consensus output must remain a function of the input SET (arms sorted by id, clusters seeded longest-fact-first), not input order. That stability is the foundation for Job 2's labeled-dataset generation. Keep the badge hex consistent across Word + frontend: High green `#1A7F37`, Medium amber `#B8860B`.
- **`backend/training/` (added 2026-06-05, Job 2):** read-only dataset builder (stdlib + PyAV; no new packages). Future maintenance MAY modify it, but the **determinism property must hold** — a rebuild must produce byte-identical `train.jsonl`/`val.jsonl` (SHA256-stable): arms sorted before clustering, OCR/transcript `set → sorted list` (never raw set order), train/val by `SHA256(seed:example_id)`. **Invocation: `cd backend && ..\.venv\Scripts\python.exe -m training.cli` — NOT from repo root** (root fails `from app.database`; if you see that ImportError, check cwd). Output `backend/training_data/` is gitignored. Do NOT start Job 3 training until the dataset is ≥20 examples.
